...
def getPersKons():
    ...
    df = pd.read_sql_query("SELECT user.*, orga.orga_name FROM perp LEFT JOIN user ON (user.id=perp.user_id) left join orga ON (user.orga_id=orga.id) WHERE kons_id='"+ konsid + "'", conn)
