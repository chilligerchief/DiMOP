...
def getMaco():
    ...
    print(request.args.get('slid'))

    df = pd.read_sql_query("SELECT maco.id, maco.stpo_id_comp_a, M1.mat_desc, S1.pos AS pos_comp_a, maco.stpo_id_comp_b, M2.mat_desc, S2.pos AS pos_comp_b, t_reltyp.`type`, t_reltyp.kind, maco.mass_product FROM maco LEFT JOIN t_reltyp on maco.t_reltyp_id=t_reltyp.id LEFT JOIN stpo AS S1 ON maco.stpo_id_comp_a=S1.id LEFT JOIN stpo AS S2 ON maco.stpo_id_comp_b=S2.id LEFT JOIN mara AS M1 ON S1.mara_id=M1.id lEFT JOIN mara AS M2 ON S2.mara_id=M2.id WHERE S1.id='"+ slid + "'", conn)
